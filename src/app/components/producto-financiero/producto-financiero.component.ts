...
import { IProducto, IRespuesta } from 'src/app/models/IProducto';
...
export class ProductoFinancieroComponent implements OnInit, AfterViewInit {
  ...
  onSearch(event: Event) {
    ...
    if (this.productos.length > 0) {
      this.productosFiltrados = this.productos.filter(producto => producto.description.toLowerCase().includes(this.searchTerm.toLowerCase()));
    }
  }
